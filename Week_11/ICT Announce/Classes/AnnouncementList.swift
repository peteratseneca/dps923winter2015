...
class AnnouncementList: UITableViewController, MWFeedParserDelegate {
    ...
    func readTheFeed() {
        ...
        // Feed URL
        let url = NSURL(string: "https://scs.senecac.on.ca/rss.xml")
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "toAnnouncementDetail" {
            ...
            let vc = segue.destinationViewController as AnnouncementDetail
            vc.title = item.title
            vc.model = self.model
            vc.item = item
        }
