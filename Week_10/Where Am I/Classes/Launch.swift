...
class Launch: UIViewController {
    ...
    func checkForDataOnDevice() {
        ...
        if model.frc_sport.fetchedObjects?.count == 0 {
            ...
            NSNotificationCenter.defaultCenter().addObserver(self, selector: "fetchCompletedSports", name: "Where_Am_I.Launch_sports_fetch_completed", object: nil)
    ...
    func fetchCompletedSports() {
        ...
        // First, listen for the notification (from the WebServiceRequest instance)
        // that indicates that the download is complete
        NSNotificationCenter.defaultCenter().addObserver(self, selector: "fetchCompletedVenues", name: "Where_Am_I.Launch_venues_fetch_completed", object: nil)
